...
def addChildren(color_artists: dict, parent: Artist):
    ...
    for artist in parent.get_children():
        # ignore empty texts
        if isinstance(artist, mpl.text.Text) and artist.get_text() == "":  # ty:ignore[possibly-missing-attribute]
            continue
        ...
        if not isinstance(artist, (mpl.text.Text, mpl.axis.XTick, mpl.axis.YTick)):  # ty:ignore[possibly-missing-attribute]
